...
function generatePDF(report) {
  return new Promise((resolve, reject) => {
    ...
    const doc = new PDFDocument({
      size: 'A4',
      margin: 15,          // Réduire les marges
      bufferPages: true    // Important pour le positionnement du footer
    });
    ...
    clientInfo.forEach((info, index) => {
      ...
      doc.font(fonts.regular)
         ...
         .fillColor(colors.black)
         .text(info.value || 'N/A', 35 + sectionWidth + spacing,
    ...
    const margin = 20;     // Réduit
    const columns = 3;
    const itemHeight = 14; // Réduit
    const itemPadding = 3; // Réduit
    const columnSpacing = 5; // Réduit
    ...
    // Ajouter le copyright en bas de la dernière page
    const addFooter = () => {
      // Obtenir les dimensions de la page
      const pageHeight = doc.page.height;
      const currentYear = new Date().getFullYear();
      
      // Calculer la position Y pour le footer (15 unités du bas)
      ...
      doc.font(textStyles.footer.font)
         ...
         .text(
           `© ${currentYear} Auto Presto. Tous droits réservés.`,
           30,
           footerY,
           {
             align: 'center',
    ...
    // Ajouter le footer sur la page courante
    addFooter();
